...
| `POST` | `/v1/ocr` | OCR 文字识别（图片 body，可选 `?model=rapidocr-ppocr-v5-mobile&max_side=1280`） |

请求中的 `model` 字段对应模型 ID。如果模型未加载，daemon 自动启动推理进程并加载模型，客户端无需手动管理。

OCR 性能相关 query 参数：

- `model`：默认 `rapidocr-ppocr-v5-mobile`，走 RapidOCR + ONNXRuntime。可选 `rapidocr-ppocr-v5-server` 走更高精度 server 模型；也兼容 PaddleOCR 模型，例如 `ppocr-v5-mobile` / `ppocr-v5-server`。
- `max_side` / `resize_long_side`：进入 OCR 前按长边等比缩图，例如 UI OCR 可从 `1280` 或 `960` 起试；传 `0` 或不传则保留原图。
- `det_limit_side_len`：检测阶段长边限制，适合继续压检测成本。
- `det_thresh` / `box_thresh` / `unclip_ratio` / `rec_thresh` / `text_score`：检测与识别阈值透传参数。
- `use_det` / `use_cls` / `use_rec` / `return_word_box` / `return_single_char_box`：RapidOCR 请求级开关；默认 `use_cls=false`。
...
| `POST` | `/api/runtime/{kind}/start` | 启动运行时（`{"modelId":"..."}`）—— kind: llm / whisper / ocr / rapidocr / faster-whisper |
